...
2. **Investigate the codebase.** For each file, symbol, signature, path, or command named in the plan's Context and
...
5. **Execute the Steps in sequential order.** Do not reorder, skip, batch ahead, or parallelize. Complete a Step's
   action and confirm it succeeded before starting the next Step.
6. **Verify the Acceptance criteria.** After the last Step, check each Acceptance criterion using the exact command or
   assertion the plan supplies. Running the project's full test suite — every test, not only the ones related to the
   change — is always one of the Acceptance criteria unless the plan or the user says not to. Record for each criterion
   the check used and its result.
7. **Write the report** to a markdown file inside the same folder as the plan. Filename: replace the trailing `PLAN.md`
...
8. **Confirm** with a one-line message naming the report file, the counts of files created, edited, and deleted, and
...
- The report is a **record of what was changed and verified**, not a plan or rationale. Do not describe WHY an edit was
  made. Do not point the reader to the plan in place of the information — no deferral references like "as in the plan",
  "see the plan", or "check the plan, line <n>".
- The report must be **self-contained**. Inline every file path, the full text of every Acceptance criterion, and the
  exact command or assertion used to check it.
- Status values: `verified` (the criterion's command or assertion ran and passed), `failed` (it ran and did not pass),
  `unverified` (it could not be run — the command cannot run in this environment, or a blocked Step prevented the check).
...
- A Step cannot be completed (missing dependency, conflicting code, failing command, or a gap the user declined to
  resolve in step 4) → stop at that Step. Do not proceed to later Steps, since they may depend on the blocked one.
  Write the report (workflow step 7) covering the files changed so far, every Acceptance criterion (criteria not
