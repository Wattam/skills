...
3. **Build the change set.** For each Step and each Acceptance criterion in the plan:
    - A unit test from step 2 already exercises the same symbol or behavior → **EDIT** (record file path and the
      assertions/cases to add, change, or remove).
    - No unit test from step 2 covers this symbol or behavior → **CREATE** (record the new file path and the
