...
## `standalone-adress-review`

Run `/standalone-adress-review` with a review file or pasted findings to validate claims, fix confirmed issues, and run checks. It accepts any code review format and optional requirements or context. With no review input, it reads `REVIEW.md` in the current working directory. It updates finding statuses in a local review file or reports them in chat for pasted reviews. It requires no pipeline documents.
