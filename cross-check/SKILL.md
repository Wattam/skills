...
1. **Locate and ingest the spec and plan.** List the folder contents. Identify the file ending with `SPEC.md` and the
...
2. **Resolve and read the pre-implementation tests.** If the folder contains `UNIT-TESTS.md` and/or
   ...
   in step 3; do not stop. If no test doc is present and the user named no test paths, treat tests as absent and skip
   the **Spec ↔ tests** and **Plan ↔ tests** cross-checks.
3. **Cross-check the documents.** Compare the documents against one another along three axes. For each finding, capture
