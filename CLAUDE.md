...
Current skills: `spec/`, `plan/`, `implement/`, `integration-tests/`, `unit-tests/`, `review-code/` (the pipeline) and
...
## The pipeline: spec → plan → {implement, integration-tests, unit-tests} → review-code

The six skills are stages of one workflow and share conventions. Preserve them when editing. The order between `plan`
...
`unit-tests` is not enforced — a TDD flow writes tests before `implement`, a non-TDD flow after.
...
3. **`implement/`** — reads the `*PLAN.md` from a given folder (or a direct path to the plan file) and **executes its
   ...
   files.
4. **`integration-tests/`** — reads the `*SPEC.md` from a given folder, creates/edits/deletes integration test files in
...
- When changing a workflow step (e.g. how gap questions are asked, how filenames are derived, the CLAUDE.md
  precondition), apply the change to all six pipeline skills so they stay consistent. Exception: `implement` writes no
